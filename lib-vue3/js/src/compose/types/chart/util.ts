...
export const toRGBA = ([r, g, b, a]: Array<number>) => `rgba(${ln(r)}, ${ln(g)}, ${ln(b)}, ${a})`
...
export const dimensionFunctions: DimensionFunctions<DimensionFunction> =
  DimensionFunctions.create<DimensionFunction>()
...
export const predefinedFilters = [
  ...
  {
    value:
      'YEAR(createdAt) = YEAR(NOW()) AND QUARTER(createdAt) = QUARTER(DATE_SUB(NOW(), INTERVAL 3 MONTH)',
  ...
  {
    value:
      "DATE_FORMAT(createdAt, '%Y-%m') = DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 1 MONTH), '%Y-%m')",
    text: 'recordsCreatedLastMonth',
  },
]

dimensionFunctions.lookup = d =>
  dimensionFunctions.find(f => d.modifier === f.value) || dimensionFunctions[0]
